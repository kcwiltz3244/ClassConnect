...
function setupSheets() {
  ...
  if (classmates.getLastRow() === 0) {
    classmates.getRange(1, 1, 1, classmateHeaders.length).setValues([classmateHeaders]);
  }
  ...
  if (events.getLastRow() === 0) {
    events.getRange(1, 1, 1, eventHeaders.length).setValues([eventHeaders]);
  }
...
function doGet(e) {
  ...
  try {
    ...
    if (action === "getClassmates") {
      ...
      result = getVisibleEvents();
    } else if (action === "lookupClassmate") {
      result = lookupClassmate(e?.parameter?.id, e?.parameter?.name);
    } else {
...
function doPost(e) {
  ...
  try {
    ...
    } else if (action === "submitEvent") {
      result = submitEvent(payload);
...
function submitEvent(data) {
  ...
  SpreadsheetApp
    .openById(SPREADSHEET_ID)
    .getSheetByName(EVENTS_SHEET)
    .appendRow([
      new Date(),
      "Approved",
      title,
      cleanText(data.type, 60),
      date,
      cleanText(data.time, 20),
      cleanText(data.location, 180),
      cleanText(data.description, 1000),
      cleanText(data.organizer, 100),
      cleanText(data.contact, 150),
      cleanText(data.rsvpUrl, 500)
    ]);
...
function getVisibleEvents() {
  ...
  const items = rows
    .filter(row => String(row[1] || "").trim().toLowerCase() === "approved")
    .map(row => ({
      dateAdded: row[0] ? new Date(row[0]).toISOString() : "",
      status: String(row[1] || ""),
      title: String(row[2] || ""),
      type: String(row[3] || ""),
      date: normalizeDateValue(row[4]),
      time: normalizeTimeValue(row[5]),
      location: String(row[6] || ""),
      description: String(row[7] || ""),
      organizer: String(row[8] || ""),
      contact: String(row[9] || ""),
      rsvpUrl: String(row[10] || "")
    }));
...
function getApprovedClassmates() {
  ...
  rows.forEach((row, index) => {
    if (String(row[1] || "").toLowerCase() !== "approved") return;
    ...
    items.push({
      ...
      email: Boolean(row[11]) ? String(row[8] || "") : "",
      phone: Boolean(row[12]) ? String(row[9] || "") : "",
      bio: String(row[10] || ""),
...
function getApprovedMessages() {
  ...
  return {
    ok: true,
    items: rows
      .filter(row => String(row[1] || "").toLowerCase() === "approved")
      .map(row => ({
        date: row[0] ? new Date(row[0]).toISOString() : "",
        author: String(row[2] || ""),
        text: String(row[3] || "")
...
function lookupClassmate(id, name) {
  ...
  if (rows.length < 2) return { ok: true, item: null };
  ...
  if (!rowNumber) return { ok: true, item: null };
...
function updateClassmate(data) {
  ...
  const rowNumber = Number(data.id || data.recordId);

  if (!rowNumber || rowNumber < 2 || rowNumber > sheet.getLastRow()) {
    throw new Error("The selected classmate record could not be found.");
  }
...
function saveProfilePhoto(dataUrl, name) {
  ...
  const filename =
    cleanText(name, 50).replace(/[^A-Za-z0-9_-]+/g, "_") +
    "_" +
    Date.now() +
    ".jpg";

  const file = folder.createFile(
    Utilities.newBlob(bytes, match[1], filename)
  );
